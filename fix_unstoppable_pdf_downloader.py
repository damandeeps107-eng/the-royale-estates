...
for fname in ['index.html', 'home.html']:
    ...
    # Replace triggerUnblockedPdfDownload block if present
    ...
    # Update brochureBtn onclick in renderInlineFolderGrid
    old_btn = "var brochureBtn = (item.brochure && item.brochure.trim()) ?"
    if old_btn in text:
        start_b = text.find(old_btn)
        end_b = text.find(';\n', text.find('📥 Download PDF Brochure</button>\';', start_b))
        if end_b != -1:
