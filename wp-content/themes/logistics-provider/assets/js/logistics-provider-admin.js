        ...
        jQuery.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'logistics_provider_dismissed_notice_handler',
              type: type,
              wpnonce: logistics_provider.wpnonce
            }
          } );
      } );
}( jQuery ) )
    ...
    if (closeBtn && detailBox) {
        closeBtn.addEventListener("click", function(e) {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=logistics_provider_dismissed_get_started_detail_notice'
            })
...
function logistics_provider_open_tab(evt, tabName) {
    let i, tabcontent, tablinks;

    // Hide all tabs
    tabcontent = document.getElementsByClassName("logistics-provider-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Remove active from all buttons
    tablinks = document.getElementsByClassName("logistics-provider-tablinks");
...
jQuery(document).ready(function ($) {
    // Hide all tabs first
    $(".logistics-provider-tabcontent").hide();
    
    // **DIRECTLY** activate first tab (no trigger)
    let firstTab = $(".logistics-provider-tablinks").first();
